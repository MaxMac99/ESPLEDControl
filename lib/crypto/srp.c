...
uint8_t crypto_verifyAndDecryptAAD(const uint8_t* key, uint8_t* nonce, uint8_t *aad, uint8_t aadLength, uint8_t* encrypted, uint8_t length, uint8_t* output_buf, uint8_t* mac)
{
    ...
    memcpy(message + aadLength + aadPadding, encrypted, length);
    ...
    {
        crypto_stream_chacha20_xor(output_buf, message + aadLength + aadPadding, length, nonce, key, 1);
        return 1;
    }
}
